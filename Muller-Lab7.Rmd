...
output:
  html_document:
    css: C:/Users/Exped/Documents/R/win-library/3.3/DATA606/labs/Lab7/lab.css
    highlight: pygments
    theme: cerulean
...
load("C:/Users/Exped/Documents/R/win-library/3.3/DATA606/labs/Lab7/more/mlb11.RData")
```
...
####(answer) I would use a scatterplot using runs and at_bats as the explanatory variable.  
...
####(answer) A moderate positive linear correlation between runs and at_bats.
...
####(answer) predicted y = 415.2389 + 1.8345 * explanatory (homeruns    )

```{r}
plot_ss(x =mlb11$homeruns, y =mlb11$runs)
cor(mlb11$runs, mlb11$homeruns)
answer = lm(runs ~ homeruns, data = mlb11)
summary(answer)
```
...
####(answer) I see a linear relationship, maybe some high variation. Not much else to say.
...
####(answer) Yes, there is a clear linear relationship, not super strong, but its there.  

```{r}
plot_ss(y = mlb11$runs, x = mlb11$wins)
cor(mlb11$runs, mlb11$wins)
model = lm(runs ~ wins, data = mlb11)
summary(model)
```
-   How does this relationship compare to the relationship between `runs` and 
    ...
    tell?
####(answer) at_bat r2 was .3729 where as with wins it is .3609712...Which means at_bat is a better predictor of runs.  
...
summary(model7)
```
####(answer) Batting average seems to have the highest R2 correlation, meaning its the best predictor of runs. 
...
summary(model10)
```
####(answer) These are all much better predictors of runs. Especially new_obs with 93% variability accounted for.

-   Check the model diagnostics for the regression model with the variable you 
    decided was the best predictor for runs.
```{r}
bestExplanatory = mlb11$new_obs
runs = mlb11$runs 
plot(bestExplanatory,runs)
abline(model9)
hist(bestExplanatory)
